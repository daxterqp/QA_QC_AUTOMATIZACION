/* Valida PRD (Proctor demo) y CAD (Cono demo) con el motor real.
 * Las celdas xref del Cono (selector/get a otro ensayo) NO se resuelven offline:
 * se omiten y las fórmulas que dependen de ellas quedan vacías (esperado).
 */
...
const auxTables = buildAux('Actividadesv1/DEMO_TablasAuxiliares.xlsx');

function run(label, file, IN, expectXrefEmpty) {
  ...
  for (const { item, spec } of mainRows) {
    ...
    spec.cells.forEach((c, i) => {
      ...
      else if (c.kind === 'xref') xrefKeys.push(key);
    });
  }
  ...
  // Errores que dependen de celdas xref (esperados offline) se filtran.
  const xrefDeps = expectXrefEmpty || [];
  const realErrors = Object.keys(errors).filter(k => !xrefDeps.includes(k));
  console.log('Celdas xref (no resolubles offline):', xrefKeys.join(',') || '(ninguna)');
  console.log('Errores REALES (excl. xref):', realErrors.length, realErrors.length ? realErrors.map(k => k + ':' + errors[k]).join(' | ') : '(ninguno) ✅');
  const secs = np.groupIntoSections(mainRows);
  console.log('Secciones · maxCols:');
  for (const s of secs) console.log('  - ' + String(s.title || '(sin)').padEnd(40) + ' maxCols=' + s.maxCols);
  return { scope, textValues, v };
}

// ── PRD (Proctor demo) ──
const prd = run('PRD — Proctor demo', 'Actividadesv1/PRD_Proctor_Demo.xlsx', {
  '1:A': '13',
  ...
  '22:A': '1103.7',
  '24:B': '10', '25:B': '20', '26:B': '30', '27:B': '40', '28:B': '50', '29:B': '60',
}, []);
console.log('  MDS(19A)=', prd.v('19A'), ' OCH(20A)=', prd.v('20A'), ' %Pasa Fondo(30E)=', prd.v('30E'));

// ── CAD (Cono demo) — 21A/22A son xref (MDS/OCH del proctor) → 24A depende de ellas.
const cad = run('CAD — Cono de Arena demo', 'Actividadesv1/CAD_ConoArena_Demo.xlsx', {
  '1:A': '28',
  '2:A': '4782', '3:A': '10', '5:A': '7800', '7:A': '2108',
  '12:A': '1', '13:A': '1029.5', '14:A': '992.6',
  '25:A': '800',
  '27:B': '0', '28:B': '0', '29:B': '5.5', '30:B': '22', '31:B': '70', '32:B': '68',
}, ['24A']);
console.log('  Dens.húmeda(11A)=', cad.v('11A'), ' Dens.seca(19A)=', cad.v('19A'), ' Vol.hueco(10A)=', cad.v('10A'), ' Hum%(18A)=', cad.v('18A'));
console.log('  (%comp 24A vacío offline porque MDS viene de xref del Proctor — se resuelve en la app al seleccionar el ensayo)');
