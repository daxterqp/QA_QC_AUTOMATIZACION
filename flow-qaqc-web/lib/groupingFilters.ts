 ...
 * agrupamientos (presets) de protocolos. Sin I/O. ESPEJO de src/utils/groupingFilters.ts.
 *
 ...
 *   op:    `=` `!=` `<` `<=` `>` `>=` `~` (`~` = contiene, texto).
 */
import type { GroupingFilterClause, GroupingCmp } from '@/types';
...
export function clauseMatches(clause: GroupingFilterClause, actual: string | number | null | undefined): boolean {
  ...
  // Fechas ISO 'YYYY-MM-DD': comparación LEXICOGRÁFICA (ordena cronológicamente).
  ...
  // `=`/`!=` numérico SOLO en celdas; en identificadores (codigo/material/...) por texto.
