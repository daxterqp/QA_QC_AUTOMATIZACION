  ...
  for (const sec of sections) {
    ...
    for (const row of sec.rows as { item: TemplateItemLite; spec: ReturnType<typeof parseNumericRow> }[]) {
      ...
      if (reportable.length === 1) {
        ...
        cols.push({ key: `${partida}:${colLetter(i)}`, label: activity, from: `key:${partida}:${colLetter(i)}`, kind: isNumericCell(c as { kind: string; mode?: string }) ? 'number' : 'text' });
      } else {
        ...
        for (const { c, i } of reportable) {
          ...
          cols.push({ key: `${partida}:${colLetter(i)}`, label: sub, group: activity, from: `key:${partida}:${colLetter(i)}`, kind: isNumericCell(c as { kind: string; mode?: string }) ? 'number' : 'text' });
        }
      }
    }
  }
