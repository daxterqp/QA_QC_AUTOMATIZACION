...
import { Loader2, Pencil, Trash2, Plus, ChevronDown } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const effectiveId = selectedId ?? (projects.length > 0 ? projects[0].id : null);
      ...
      <div className="flex-1 max-w-2xl w-full mx-auto px-4 py-5 flex flex-col gap-4">
        ...
        {loadingProjects ? (
          ...
          <div className="relative">
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-modal z-20 py-1 border border-border overflow-hidden">
                {projects.map(p => (
                  <button
                    key={p.id}
        ...
        {/* Dashboard content */}
        {effectiveId && <ProjectDashboard key={effectiveId} projectId={effectiveId} />}
      </div>
    </div>
  );
}
