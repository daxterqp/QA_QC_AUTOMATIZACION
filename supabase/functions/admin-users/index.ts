  ...
  try {
    ...
    if (action === "create") {
      ...
      const { error: insErr } = await admin.from("users").insert({
        id: uid, auth_id: uid, email, name, apellido: apellido ?? null, role, is_active: true,
        org_id: callerOrgId, created_at: now, updated_at: now,
      });
      ...
      if (Array.isArray(projectIds) && projectIds.length > 0) {
        const rows = projectIds.map((pid: string) => ({ id: crypto.randomUUID(), user_id: uid, project_id: pid, org_id: callerOrgId, created_at: now, updated_at: now }));
    ...
    if (action === "update") {
      ...
      const { data: target } = await admin.from("users").select("auth_id, org_id").eq("id", userId).maybeSingle();
      if (!target) return json({ error: "usuario no encontrado" }, 404);
      // service_role bypassa RLS → la frontera de org se enforcea ACÁ (evita account-takeover cross-empresa).
      if ((target as { org_id?: string | null }).org_id !== callerOrgId) return json({ error: "forbidden: usuario de otra organización" }, 403);
      ...
      await admin.from("users").update(patch).eq("id", userId).eq("org_id", callerOrgId);
    ...
    if (action === "delete") {
      ...
      if (!target) return json({ error: "usuario no encontrado" }, 404);
      if ((target as { org_id?: string | null }).org_id !== callerOrgId) return json({ error: "forbidden: usuario de otra organización" }, 403);
      await admin.from("users").update({ is_active: false, updated_at: now }).eq("id", userId).eq("org_id", callerOrgId);
      return json({ ok: true });
    }
