...
export interface LinearSubtramoProgress {
  ...
  done: number;      // Σ min(aprobados, requerido) sobre los tipos del juego
  total: number;     // = juegoSize
  complete: boolean; // todos los tipos alcanzaron su cantidad
}
...
/**
 * @param subtramoLenM   longitud de subtramo (m).
 * @param testSet        juego de ensayos uniforme [{id_protocolo, count}].
 * @param tramos         tramos con progresivas (station_start/end).
 * @param protocols      ensayos del proyecto (con sectorId, subtramoIndex, idProtocolo, status).
 */
export function computeLinearProgress(
  ...
  // Tramos válidos (con progresivas) + nº de subtramos de cada uno.
  ...
  // Avance acumulado + detalle por subtramo.
  let approved = 0;
