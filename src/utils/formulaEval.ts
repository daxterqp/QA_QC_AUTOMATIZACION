...
function evalNode(n: Node, env: Env): number {
  switch (n.type) {
    ...
    case 'xref': {
      // En móvil `xrefValues` típicamente NO se provee (el hook useXrefValues
      // es solo web). Diferenciamos los dos casos para no engañar al usuario:
      //   - xrefValues undefined → feature no disponible en esta plataforma
      //   - xrefValues definido pero sin la key → realmente no encontrado/aprobado
      if (!env.xrefValues) {
        throw new FormulaError('xref-unsupported',
          `Referencia cruzada @${n.externalId}.${n.key} no soportada offline — abre el protocolo desde la versión web.`);
      }
